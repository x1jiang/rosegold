...
def calculate_concordance_metrics(df_merged: pd.DataFrame) -> Dict[str, Any]:
    ...
    if "human_positive" not in df_merged.columns or "llm_positive" not in df_merged.columns:
        return {
            "total_evaluated": 0,
            "overall_agreement_pct": 0.0,
            "cohens_kappa": 0.0,
    ...
    valid_mask = df_merged["human_positive"].notna() & df_merged["llm_positive"].notna()
    ...
    y_true = df_merged.loc[valid_mask, "human_positive"].astype(bool).values
    y_pred = df_merged.loc[valid_mask, "llm_positive"].astype(bool).values
