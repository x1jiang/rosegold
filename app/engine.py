    ...
    def _mock_adjudicate(self, records: List[Dict[str, Any]], target_condition: str) -> List[Dict[str, Any]]:
        ...
        for rec in records:
            ...
            if "sepsis" in cond_lower or "septic" in cond_lower:
                if "septic shock" in text or "lactate 4.2" in text or "norepinephrine" in text or "mssa sepsis" in text or "bacteremia" in text or "urosepsis" in text:
                    status = "CONFIRMED_POSITIVE"
                    present = True
                    conf = 0.96
                    criteria = ["SIRS/qSOFA criteria met", "Confirmed bacterial source", "Acute organ dysfunction or bacteremia"]
                    evidence = [{
                        "note_id": rec.get('notes_formatted_text', '').split("Note ID: ")[1].split(" ")[0] if "Note ID: " in rec.get('notes_formatted_text', '') else 30001,
                        "note_date": rec.get('visit_start_date', '2026-03-01'),
                        "evidence_quote": "Patient admitted with fever, bacteremia/elevated inflammatory markers and organ dysfunction.",
                        "interpretation": "Meets clinical criteria for Sepsis"
                    }]
                    rationale = "Encounter documentation confirms bacterial infection with systemic inflammatory response and organ dysfunction."
                elif "pneumonia" in text and "uncomplicated" in text:
                    ...
                    evidence = [{
                        "note_id": 30008,
                        "note_date": rec.get('visit_start_date', '2026-03-12'),
                        "evidence_quote": "Stable hemodynamics, normal lactate, no evidence of acute organ dysfunction or sepsis-3 criteria.",
                        "interpretation": "Infection present without organ dysfunction (Sepsis-3 negative)"
                    }]
                    rationale = "Community-acquired pneumonia successfully treated without acute organ dysfunction or hemodynamic collapse."
                elif "trauma" in text or "rib fracture" in text:
                    status = "CONFIRMED_NEGATIVE"
                    present = False
                    conf = 0.94
                    criteria = []
                    evidence = [{
                        "note_id": 30010,
                        "note_date": rec.get('visit_start_date', '2026-03-18'),
                        "evidence_quote": "Non-infectious SIRS physiology driven by acute traumatic tissue injury and pain. No infection.",
                        "interpretation": "Non-infectious post-traumatic SIRS mimic"
                    }]
                    rationale = "Tachycardia and leukocytosis driven by acute trauma and pain rather than infectious etiology."
                else:
                    status = "CONFIRMED_NEGATIVE"
                    present = False
                    conf = 0.98
                    criteria = []
                    evidence = []
                    rationale = f"No clinical, diagnostic, or microbiological evidence of {target_condition} found in chart."

            # Stroke Condition
            elif "stroke" in cond_lower or "infarct" in cond_lower:
                if "stroke" in text or "nihss" in text or "occlusion" in text:
                    status = "CONFIRMED_POSITIVE"
                    present = True
                    conf = 0.95
                    criteria = ["Sudden onset focal neurological deficit", "CTA confirmed M1 MCA occlusion", "IV TNK + Mechanical Thrombectomy performed"]
                    evidence = [{
                        "note_id": 30006,
                        "note_date": rec.get('visit_start_date', '2026-03-10'),
                        "evidence_quote": "CT Angiogram: Dense occlusion of the proximal left M1 segment of MCA. Met criteria for IV TNK and emergent mechanical thrombectomy.",
                        "interpretation": "Definitive clinical and angiographic proof of acute ischemic stroke"
                    }]
                    rationale = "Acute ischemic stroke with large vessel occlusion confirmed by clinical presentation, neuroimaging, and successful thrombectomy."
                else:
                    status = "CONFIRMED_NEGATIVE"
                    present = False
                    conf = 0.99
                    criteria = []
                    evidence = []
                    rationale = "No documentation of acute neurological deficits or ischemic cerebral injury."
            else:
                status = "CONFIRMED_NEGATIVE"
                present = False
                conf = 0.90
                criteria = []
                evidence = []
                rationale = f"No indicators for {target_condition} found."
            ...
            results.append({
                ...
                'adjudication_timestamp': timestamp,
                'inference_backend': 'keyword_rules',
            })
        return results
